...
MAX_RANDOM = sys.maxsize # converted to hex goes to 8 chars (at least, in Python 2.3)
rand = random.Random()
ip = ''
...
hexip = ''.join(["%04x" % int(i) for i in ip.split('.')]) # leave space for ip v6 (65K in each sub)
...
class CircularSet:
  ...
  def add(self, val):
    ...
    assert val not in self.queue_map, 'This value is already in the set!'
...
def generate():
  ...
  try:
    ...
    while 1:
      parts = []
      # time part
      parts.append("%016x" % (int(time.time() * 1000)))
      # ip part
      parts.append(hexip)
      # random part
...
def extract_time(guid):
  ...
  return float(int(guid[0:16], 16)) / 1000
