...
from traitsui.api import View,Item,VGroup,HGroup, CodeEditor
...
class RangeSelectionTools(HasTraits):
    ...
    def _on_trace_text(self):
        print("DEBUG _on_trace_text", __file__)
        assert True, "_on_trace_text **** hard code start end time stamp"
        self.start=20271178456
        self.end=20271193816
        if self.end != self.start:
...
class tcPlot(BarPlot):
    ...
    def _gather_timechart_points(self,start_ts,end_ts,y,step):
        ...
        points = column_stack((start_ts,end_ts,
                               zeros(high_i-low_i)+(y+step), ones(high_i-low_i)+(y-step),array(list(range(low_i,high_i)))))
        return points
    ...
    def _draw_plot(self, gc, view_bounds=None, mode="normal"):
        ...
        if len(four_pixels) == 1:
            self.overview_threshold = 1<<int(log(1+int(four_pixels[0] - four_pixels[0]),2))
        else:
